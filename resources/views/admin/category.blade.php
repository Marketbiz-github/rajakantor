...
@section('content')
            ...
            <div class="flex justify-between items-center mb-6">
                <div class="flex items-center gap-2">
                    ...
                    <button type="button" onclick="document.getElementById('helpModal').classList.remove('hidden')"
                        class="text-gray-400 hover:text-blue-600 transition-colors" title="Informasi Penghapusan Kategori">
            ...
            <form method="GET" action="{{ route('category.index') }}" class="mb-4 flex flex-wrap gap-2">
                ...
                @if(request('search') || request('status'))
                    <a href="{{ route('category.index') }}"
                        class="bg-gray-400 hover:bg-gray-500 text-white px-4 py-2 rounded flex items-center">Reset</a>
                @endif
            </form>
                                ...
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="p-4 text-sm text-gray-700">
                                        <input type="checkbox" name="ids[]" value="{{ $item->id }}"
                                            ...
                                            data-child="{{ $item->child_count }}" data-product="{{ $item->product_count }}"
                                            data-children="{{ $item->children_names }}">
                                    </td>
                                    <td class="p-4 text-sm text-gray-700">{{ $categories->firstItem() + $index }}</td>
                                    <td class="p-4 text-sm text-gray-700">{{ $item->id_category }}</td>
                                    <td class="p-4 text-sm text-gray-900 font-medium whitespace-normal min-w-[200px]">
                                        {{ $item->name }}
                                    </td>
                                    ...
                                    <td class="p-4 text-sm">
                                        @if($item->status == 1)
                                            <span
                                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium text-green-700 bg-green-100 border border-green-200">Aktif</span>
                                        @else
                                            <span
                                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium text-red-700 bg-red-100 border border-red-200">Tidak
                                                Aktif</span>
                                        @endif
                                    </td>
                                    <td class="p-4 text-sm text-gray-700">
                                    ...
                                    <td class="p-4 text-sm flex gap-3">
                                        <a href="{{ url('/dashboard/category/' . $item->id . '/edit') }}"
                                            class="text-teal-600 hover:text-teal-800 font-medium"><i
                                                class="fas fa-edit mr-1"></i>Edit</a>
                                        <a href="{{ url('/category/' . $item->id_category . '-' . $item->slug) }}"
                                            target="_blank" class="text-blue-600 hover:text-blue-800 font-medium"><i
                                                class="fas fa-external-link-alt mr-1"></i>View</a>
                                        <button type="button"
                                            onclick="deleteCategory({{ $item->id }}, {{ $item->child_count }}, {{ $item->product_count }}, '{{ htmlspecialchars($item->children_names ?? '', ENT_QUOTES) }}')"
                                            class="text-red-600 hover:text-red-800 font-medium"><i
    ...
    <!-- Modal Peringatan -->
    <div id="alertModal"
        class="fixed inset-0 lg:left-72 z-[9999] hidden flex items-center justify-center bg-black bg-opacity-50">
        <div class="bg-white rounded-lg shadow-lg w-[90%] md:w-1/3 p-6">
            ...
            <div class="flex justify-end">
                <button type="button" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded"
                    onclick="document.getElementById('alertModal').classList.add('hidden')">Tutup</button>
            </div>
        </div>
    </div>
    ...
    <div id="helpModal"
        ...
        <div class="bg-white rounded-lg shadow-lg w-[90%] md:w-1/2 p-6">
            ...
            <div class="text-gray-700 space-y-3 mb-6 text-sm">
                <p>Menghapus kategori memiliki aturan khusus untuk menjaga agar data produk Anda tidak hilang secara tidak
                    sengaja:</p>
                <ul class="list-disc pl-5 space-y-2 px-3">
                    <li><strong class="text-red-600">Proteksi Subkategori:</strong> Anda tidak bisa menghapus kategori yang
                        masih memiliki subkategori (kategori turunan). Hapus subkategorinya terlebih dahulu.</li>
                    <li><strong class="text-yellow-600">Opsi Hapus Produk:</strong> Jika kategori yang akan dihapus memiliki
                        produk, Anda akan diberikan 2 pilihan:
                        <ul class="list-circle pl-5 mt-1 space-y-1">
                            <li><strong>Hanya hapus kategori:</strong> Kategori akan terhapus, tetapi produk akan tetap ada
                                di database (hanya dilepas dari kategori ini).</li>
                            <li><strong class="text-red-600">Hapus beserta produk:</strong> Kategori beserta <u>seluruh
                                    produk</u> di dalamnya akan dihapus secara permanen, termasuk seluruh gambarnya.
                                <b>Gunakan opsi ini dengan sangat hati-hati!</b>
                            </li>
                        </ul>
                    </li>
                </ul>
            ...
            <div class="flex justify-end">
                <button type="button" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded"
                    onclick="document.getElementById('helpModal').classList.add('hidden')">Mengerti</button>
            </div>
        </div>
    </div>
    ...
    <div id="deleteModal"
        ...
        <div class="bg-white rounded-lg shadow-lg w-[90%] md:w-1/3 p-6">
            ...
            <div id="deleteOptions" class="hidden mb-6 bg-yellow-50 border border-yellow-200 p-4 rounded-lg">
                <p class="text-sm text-yellow-800 mb-3 font-medium">Kategori ini masih berisi produk. Pilih tindakan untuk
                    produk tersebut:</p>
                <label class="flex items-start gap-2 mb-2 cursor-pointer">
                    <input type="radio" name="product_action" value="0" class="mt-1" checked>
                    <span class="text-sm text-gray-700">Hanya hapus kategori (produk tetap ada, relasi kategori
                        dilepas)</span>
                </label>
            ...
            <div class="flex justify-end">
                <button type="button" class="bg-gray-400 hover:bg-gray-500 text-white px-4 py-2 rounded mr-2"
                    onclick="document.getElementById('deleteModal').classList.add('hidden')">Batal</button>
